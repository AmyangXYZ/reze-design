...
- **Lip sync from the lyrics** — one click turns the `.lrc` into a mouth-morph VMD: kana, romaji, Korean, Chinese and pinyin map by rule onto the five MMD mouth shapes, timed to each line and articulated per syllable. It attaches to the model's morph slot and downloads as a standard VMD any MMD tool can read.
