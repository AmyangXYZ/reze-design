...
**改变全局最快的一步：按 ⌘K，输入渲染风格。**三套内置风格——*深空之眼*、*鸣潮* 与
*绝区零*——各自是一整套观感，选中其一会按角色逐组替换：身体组换成那套的身体观感，
头发组换成它的头发观感。那套风格没有对应角色的组会原样保留，因此舞台材质、你自己搭的
图、以及中性默认图都能安然度过一次切换。

有两项场景设置会跟着一起走，因为它们本就属于这套观感、而不只是摆在旁边：**视图变换**
（鸣潮是在 Standard 下调的，深空之眼与绝区零都是 Filmic、曝光不同，互换着看就是
另一张画面），以及**世界光**——每个表面都会把环境光乘进去，为一套调好的世界色会和
另一套打架。绝区零把后一件事摆到了明处：它的分层是对着三套里最暗的世界调的，放到
深空之眼的洋红世界下，无论分层本身多完整，每个表面都会读成全亮。太阳不动：主光放在
哪里属于布光，切换风格没有理由去挪它。
