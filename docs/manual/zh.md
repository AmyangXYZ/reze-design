...
它由自研引擎驱动——reze-engine，一个为 MMD 而写的 WebGPU 渲染器——平台的能力边界正来
自于此：材质是直接编译为 WGSL 的节点图，场景特效是对着实时场景即写即用的着色器，动作、
表情与镜头都能在时间轴上编辑，视频导出以离线方式逐帧推进时钟，任何机器上渲染 4K 60 帧
都能得到逐像素一致的结果。围绕引擎的是一个跨会话保留工作的编辑器，以及一个平台：每个
发布的场景都是一个可打开、可转动、可二创的实时页面。

手册的顺序就是你工作的顺序：**第 0 节**是背景，写给没有接触过 MMD 的读者。**第 1 节**
把完整流程走一遍——导入素材、取景、打光、定风格、改动作、导出、发布。**第 2 节**深入三
个创作面——调色预设、WGSL 场景特效、节点材质图——每一节都给出完整契约，读完即可独立
创作。模型、动作与音乐的来源见[附录 B](#附录-b-去哪里找模型动作和音乐)；
[附录 D](#附录-d-着色图节点速查) 是生成一张图时要照着写的节点注册表。
...
从后往前共四层。最底层是**背景色**；其上三选一：**backdrop**（平铺在场景之后的一张
图）、**skybox**（2:1 等距柱状全景，投影成无限远的天穹，随镜头转动，转动视角如同望向
真实环境），或 **plate**（实拍素材，角色不是站在它前面，而是站在它*里面*）。plate 与
其说是一个文件，不如说是一种声明：它表示镜头、地面与光线都应当与这段素材对齐，因此它会
把当初配好的镜头与地面位置一并带回来；它也可以是斜的——手机拍的素材本就会歪，站进去的
场景自然要跟着歪。最上层是**场景特效**——一段可以画在角色前后两侧的着色器。星空与极光画在身后；雨、
...
**道具。**话筒、扇子、伞、剑——角色手里拿的、身上戴的 PMX 物件是**道具**，位于
**Scene → Objects → Props**，与平面同处一行。上传方式与舞台相同（⌘K 输入*上传道具 PMX*，
或点该标签页自己的按钮）；载入后立在原点，带自己的缩放、位置与旋转。**绑定到**把它挂在
某位角色的骨骼上——即 MMD 的外部親——骨骼列表把常用的排在前面：左右手首、头、首、
上半身、センター、左右足首，其后才是整副骨架。挂上之后，位置与旋转滑杆是**该骨骼空间里
的偏移**，说明行写明是哪根骨骼，数值也从零重新开始。话筒起初会顺着小臂躺着——道具的
长度沿它自己的 Y 轴，而手首骨骼的轴顺着手臂——转一下就到了手里。缩放全程含义不变。
道具保留物理与描边（手机挂饰会晃，而且她歪手时晃的方向是对的），但从不算作角色：剪影
类特效描的仍是她而不是话筒；它也不会像舞台那样顶掉地面。这里的一切都是场景状态，会自
动保存并随发布一起带走。

## 1.7 风格

**改变全局最快的一步：按 ⌘K，输入渲染风格。**四套内置风格——*深空之眼*、*鸣潮*、
*绝区零* 与 *崩坏：星穹铁道*——各自是一整套观感，选中其一会按角色逐组替换：身体组换成
那套的身体观感，头发组换成它的头发观感。那套风格没有对应角色的组会原样保留，因此舞台
材质、你自己搭的图、以及中性默认图都能安然度过一次切换。

跟着一起走的，只有这套分层「读得出来」所必需的东西。**视图变换**与曝光——鸣潮与绝区零
是在 Standard 下调的，深空之眼与星穹铁道则是 Filmic，互换着看就是另一张画面。**世界
光**——每个表面都会把环境光乘进去，为一套调好的世界色会和另一套打架。以及，当一套风格
确实需要时，它自己的**主光**：绝区零的分层是对着真正照到它的光量化的，所以它会带上自己
的太阳——强度 2.0，来自身后偏上——以及一层低而宽的辉光。

你的背景色、描边、调色与地面都留在原处。那些属于布光与取景，切换渲染风格没有理由替你
重新布置画面。
...
**特效。** 特效标签页列出场景里的特效，每个一条轨道。轨道上的一个片段就是这个特效
播放的一段时间，特效自己的时钟从片段左端开始走。拖动片段移动它，拖动一端裁剪它，
复制粘贴——⌘C 和 ⌘V，或者右键菜单——就能让同一个特效再触发一次；粘贴落在播放头
处，或者其后第一个空位。没有安排时间的特效贯穿整个场景，显示为从第一帧开始的一整
块。同一条轨道上的片段不会重叠，删掉最后一段，这个特效也随之移除。
...
**分享导出统计**是渲染按钮下方的一个开关，默认关闭。打开后，一次成片会上报它的分辨率与
画面比例、画面中模型的文件名，以及用到了哪些特效、材质节点图与调色。其中不含任何身份信息
——无论是否登录，这些记录都不带账号——你的动作、音乐、场景与视频本身也始终留在本机。汇总
结果公开在 [reze.design/analysis](https://reze.design/analysis)；
[reze.design/privacy](https://reze.design/privacy) 则列出了上报什么、以及始终不上报
什么。
...
| *Finger Shapes* | 两条轮廓：一条始终包住双手的外轮廓，一条是手指围出的空间、按围合程度淡入——在正对相机的平面上把手指变成胶囊体，起点沿离手指最远的方向爬升，射线打在加粗过、能封住窄缝的胶囊上来判断，再沿真实手指边缘描出轮廓，缝隙处用直线接上 |
...
| *Subtitles* | 同一时间戳上的多行——双语 `.lrc` 的原文与译文——按“与当前行同 start 的连续行”找出来，叠成一条字幕 |
...
graph**。内置图分三套，外加一个中性底子，都是可以直接 fork 的参考实现：
...
- **WuWa**——*WuWa Body*、*WuWa Cloth*、*WuWa Hair*、*WuWa Face*、*WuWa Metal*、
  *WuWa Eye*。每张二十一个节点，不走闭包而是直接用光：半兰伯特穿过一段很窄的软阈值，
  ...
  转折的二次元观感就从这里起步。
- **ZZZ**——*ZZZ Body*、*ZZZ Cloth*、*ZZZ Eye*、*ZZZ Face*、*ZZZ Hair*、
  *ZZZ Metal*。一套配方配六组数字，源 .blend 本身就是这么搭的：把光照闭包量化成一张
  遮罩，再把贴图拆成受光与背光两支、各自上色，最后透过这张遮罩乘回去。身体、脸和
  头发用 smoothstep 转折，衣服、眼睛和金属用三阶硬台阶。头发额外叠一个
  `sphere_map`——源文件的高光就在这里，因此头发材质本身不带球面贴图的模型，头发会
  显得平。想知道一整套观感最少能用多少节点写完，就读这一套。
中性底子不在库里：未分组材质本来就是它，新建组和新建图也都从它开始，把它摆进货架
等于给空白页做一份副本。

三套都不自带任何贴图。预设读的是材质本身的贴图与球面贴图，因此可以用在任何模型
上——你自己发布预设时，这一点值得保持。
...
重调。以及**视图变换由你决定**：Post 行的 Tone 里有 Standard、Filmic 与 AgX，一份在
其中某一种下渲染的 `.blend`，在你把它对上之前是不会一致的。二次元与 NPR 常用
Standard——图算出什么颜色，落到屏幕上就是什么颜色——但要对的是源文件，不是惯例。
这里的 AgX 就是 Blender 自己的 `AgX_Base_sRGB` 立方体，因此一份跑在基础 AgX 下的
`.blend` 能原样搬过来；叠在它上面的那些 Look（包括 High Contrast）则没有对应物。
...
- **再次发布即覆盖。** 在自己的条目上再次发布，直接覆盖原来的内容，不会另存一个版本 *n+1* 让旧的继续留着。
- **场景跟随条目。** 场景记录的是 `{ id }`，别人打开场景时解析到的就是这个条目当下的样子。你下个月把这套预设重调一遍，别人上个月用它发布的场景也跟着变。

因此发布是一种承诺，而且是一种活的承诺：修好一个预设，用到它的每个场景都会跟着修好——改坏了也一样。
...
**WGSL**——WebGPU 着色语言。场景特效用它编写，着色图编译为它。
