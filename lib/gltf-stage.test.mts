...
// W/m² and the cast's fill. Those numbers are Unity's, with no π; the app
// takes lamps and sun at π times them, into the engine's Blender convention.
...
for (const lamp of rig.lamps) {
  ...
  assert.ok(Math.abs(twin.intensity * Math.PI - lamp.intensity) / Math.max(twin.intensity, 1e-6) < 0.01, `${lamp.name} intensity ${lamp.intensity} vs π·${twin.intensity}`)
...
assert.ok(Math.abs(rig.sun.strength - RIG_SUN.strength * Math.PI) / RIG_SUN.strength < 0.01, `sun strength ${rig.sun.strength} vs π·${RIG_SUN.strength}`)
