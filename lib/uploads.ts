...
function unicodePathFromExtra(u8: Uint8Array, start: number, len: number): string | null {
  ...
  while (p + 4 <= len) {
    ...
    if (id === UNICODE_PATH_EXTRA && size >= 5) {
      ...
      return new TextDecoder("utf-8").decode(u8.subarray(start + p + 4 + 5, start + p + 4 + size))
    }
    p += 4 + size
  }
  return null
  ...
  for (const enc of candidates) {
    ...
    for (const bytes of nameBytes) {
      ...
      for (const ch of s) {
        ...
        if (c === 0xfffd) score += 20 // undecodable — strong evidence against
        else if (c >= 0xff61 && c <= 0xff9f) score += 2 // halfwidth katakana (mojibake smell)
...
export async function unzipToFiles(zip: File): Promise<File[]> {
  ...
  type Entry = { name: string | null; bytes: Uint8Array; method: number; compSize: number; localOff: number }
  const entries: Entry[] = []
  ...
  for (let n = 0; n < count; n++) {
    ...
    let name: string | null = unicodePathFromExtra(u8, off + 46 + nameLen, extraLen)
    if (name === null && (flags & 0x800) !== 0) name = new TextDecoder("utf-8").decode(bytes)
    if (name === null) {
      try {
  ...
  for (const e of entries) {
    ...
    } else throw new Error(`Unsupported zip compression (${e.method}) in ${zip.name}`)
