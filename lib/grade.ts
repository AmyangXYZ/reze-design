...
/**
 * Is this parsed JSON a grade spec?
 *
 * Checked rather than cast, because these files are meant to be written by hand
 * and generated by tools, and a spec that is wrong in one field would otherwise
 * reach the sliders as NaN and quietly grade a scene to nothing. Five fields of
 * finite numbers is the whole contract, so verifying it costs nothing.
 */
export function isGradeSpec(value: unknown): value is GradeSpec {
  if (!value || typeof value !== "object") return false
  const v = value as Record<string, unknown>
  const range = (r: unknown) =>
    Array.isArray(r) && (r.length === 2 || r.length === 3) && r.every((n) => typeof n === "number" && Number.isFinite(n))
  const num = (n: unknown) => typeof n === "number" && Number.isFinite(n)
  return range(v.shadows) && range(v.midtones) && range(v.highlights) && num(v.contrast) && num(v.saturation)
}
