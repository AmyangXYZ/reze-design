...
function HexField({ value, onChange }: { value: string; onChange: (hex: string) => void }) {
  ...
  return (
    <input
      value={text}
      spellCheck={false}
      className="h-7 w-28 rounded-md border border-white/10 bg-black/30 px-2 font-mono text-xs text-zinc-300 outline-none focus:border-pink-400/50"
      onChange={(e) => {
  ...
  return (
    <>
      <span className="flex items-center gap-1.5">
        ...
        />
        <span className="w-14 font-mono text-xs text-zinc-500">{active}</span>
      </span>
